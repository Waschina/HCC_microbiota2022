...
deg_stats <- deg_stats[GeneID != "ENSG00000273730"] # pseudogene
...
genus_keep <- names(which(apply(liver_counts_norm,1,
                                function(x) sum(x > min(liver_counts_norm)) >= 5)))
...
stat_TM[!is.na(pvalue), padj := p.adjust(pvalue)]
...
stat_TM_small[dir == "HCC upreg.", GeneID2 := paste0("<b style='color:#d7191c'>",GeneName,"</b>")]
stat_TM_small[dir == "NAFLD upreg.", GeneID2 := paste0("<b style='color:#2c7bb6'>",GeneName,"</b>")]
stat_TM_small[dir == "no diff. expr.", GeneID2 := paste0("<b style='color:#000000'>",GeneName,"</b>")]
...
genus_order <- cluster_factors(stat_TM_small, by.x = "genus", by.y = "GeneID2",
                             value = "tmp")
gene_order <- cluster_factors(stat_TM_small, by.x = "GeneID2", by.y = "genus",
                              value = "tmp")

stat_TM_small$GeneID2 <- factor(stat_TM_small$GeneID2, levels = gene_order)
