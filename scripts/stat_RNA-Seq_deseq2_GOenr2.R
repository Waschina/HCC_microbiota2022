...
rsi <- fread("data/rnaseq/salmon.merged.gene_tpm.tsv")
rsi <- melt(rsi, id.vars = c("gene_id","gene_name"), variable.name = "name",
            value.name = "TPM")
rsi[, name := gsub("\\..*$","",name)]
rsi <- rsi[!grepl("_PAR_Y$",gene_id)]
rsi[, gene_id := gsub("\\..*$","",gene_id)]
rsi <- rsi[,.(name, `Gene ID` = gene_id, `Gene Name` = gene_name, TPM)]
...
colnames(cts) <- str_sub(colnames(cts), 1, 6)
cts <- cts[rownames(cts) %in% rel_genes,]
cts <- round(cts)
...
ggsave("output/plots/rna_GOenrichments_compared_to_NAFLD.pdf",p_go_comb,
       width = 4, height = 11)
ggsave("output/plots/submission1/FigS3.pdf",p_go_comb,
       width = 4, height = 11)
